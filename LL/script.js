...
function showValidationModal(validationResult, onComplete) {
  ...
  document.getElementById("applyFixesBtn").onclick = () => {
    ...
    logCorrections("ll", overlay._loggedFixes, overlay._data, "applied");
    onComplete(overlay._data);
  };

  document.getElementById("skipValidationBtn").onclick = () => {
    overlay.classList.remove("active");
    logCorrections("ll", overlay._loggedFixes, validationResult.processed_rows, "skipped");
...
function buildNormalLabel(div, row, logo) {
  ...
  div.innerHTML = `
    ...
    <div class="line4">cikkszám: <span data-edit="Cikkszám">${escapeAttr((row["Cikkszám"] || "").substring(0, 16))}</span></div>
