...
function showValidationModal(validationResult, onComplete) {
  ...
  document.getElementById("applyFixesBtn").onclick = () => {
    ...
    logCorrections("ditall", overlay._loggedFixes, overlay._data, "applied");
    onComplete(overlay._data);
  };

  document.getElementById("skipValidationBtn").onclick = () => {
    overlay.classList.remove("active");
    logCorrections("ditall", overlay._loggedFixes, validationResult.processed_rows, "skipped");
...
function buildNormalLabel(div, row, logo) {
  ...
  // A megjelenített mező értéke szerkeszthető (data-edit a ténylegesen mutatott mezőre).
  // Üres esetben a Negyedik_sor szerkeszthető (így 4. sor hozzáadható).
  let line4Content = "";
  const cikkszam = row["Cikkszám"] || "";
  const szin = row["Szín"] || "";
  const negyedikSor = row["Negyedik_sor"] || "";
  if (szin) {
    line4Content = `Szín: <strong><span class="line4-val" data-edit="Szín">${escapeAttr(szin)}</span></strong>`;
  } else if (cikkszam) {
    line4Content = `cikkszám: <span class="line4-val" data-edit="Cikkszám">${escapeAttr(cikkszam.substring(0, 14))}</span>`;
  } else {
    line4Content = `<span class="line4-val" data-edit="Negyedik_sor">${escapeAttr(negyedikSor.substring(0, 24))}</span>`;
  }
  ...
  div.innerHTML = `
    ...
    <div class="line1" style="font-weight:${nameWeight}" data-edit="Első_sor">${escapeAttr(line1)}</div>
    <div class="line2" style="font-weight:${nameWeight}" data-edit="Második_sor">${escapeAttr(secondLineText)}</div>
    <div class="line3" style="font-weight:${nameWeight}" data-edit="Harmadik_sor">${escapeAttr(thirdLineText)}</div>
    <div class="kiszereles" data-edit="Kiszerelés">${formatKiszereles(kiszereles)}</div>
...
function buildSaleLabel(div, row, logo) {
  ...
  div.innerHTML = `
    ...
    <div class="sale-info-row">
      <span class="sale-cikk" data-edit="Cikkszám">${escapeAttr((row["Cikkszám"] || "").substring(0, 12))}</span>
      <span class="sale-kiszeres" data-edit="Kiszerelés">${formatKiszereles(kiszereles)}</span>
    </div>
...
function renderLabels(data) {
  ...
  data.forEach((row, index) => {
    ...
    if (eanCode) {
      ...
      // letakarja a bal oldali szövegmezőket (megnevezés sorok, 4. sor) → nem kattinthatók.
      barcodeSVG.remove();
    }
...
(function initCimbi() {
  ...
  const TEXT_COLS = ["Első_sor", "Második_sor", "Harmadik_sor", "Negyedik_sor", "Szín"];
  ...
  const FIELD_COLS = ["Cikkszám", "EAN-13", "Kiszerelés", "Első_sor", "Második_sor", "Harmadik_sor", "Negyedik_sor", "Szín", "Ár", "Akciós_ár"];
  const FIELD_ALIASES = {
  ...
  // Ditall: 4 soros tördelés (vagy 3, ha a 4. sor a Szín); line3/4 limit 22 (kisbetűsnél +2).
  function computeNameLines(value, row) {
    const upper = isUpperName(value);
    const mc = upper ? 18 : 20;
    const mc3 = upper ? 22 : 24;
    const hasSzin = String(row["Szín"] == null ? "" : row["Szín"]).trim() !== "";
    return splitName(value, hasSzin ? 3 : 4, mc, mc3);
  }
  function applyNameToRow(row, value) {
    ...
    row["Negyedik_sor"] = hasSzin ? "" : (lines[3] || "");
  }
  ...
  function rowText(row) {
    return [row["Első_sor"], row["Második_sor"], row["Harmadik_sor"], row["Negyedik_sor"], row["Szín"], row["Cikkszám"]]
  ...
  const NAME_LINE_COLS = ["Első_sor", "Második_sor", "Harmadik_sor", "Negyedik_sor"];
...
(function initInlineEdit() {
  ...
  function readInto(idx, labelEl) {
    ...
    labelEl.querySelectorAll("[data-edit]").forEach(el => {
      ...
      // 4. sor (Negyedik_sor) prefix-felismerés: ha a felhasználó "Szín: ..." vagy
      // "cikkszám: ..." formában gépel, alakítsuk át aktív Szín/Cikkszám mezővé.
      // A renderLabels ezután a megfelelő stílust adja (Szín értéke vastag, prefix előre).
      if (field === "Negyedik_sor") {
        const m = val.match(/^\s*(sz[ií]n|color|cikksz[áa]m|cikk)\s*[:：]\s*(.+)$/i);
        if (m && m[2].trim()) {
          const key = /^(sz|co)/i.test(m[1]) ? "Szín" : "Cikkszám";
          const newVal = key === "Cikkszám" ? m[2].trim().substring(0, 14) : m[2].trim();
          if (String(validatedData[idx][key] == null ? "" : validatedData[idx][key]) !== newVal) {
            validatedData[idx][key] = newVal; changed = true;
          }
          if (String(validatedData[idx]["Negyedik_sor"] == null ? "" : validatedData[idx]["Negyedik_sor"]) !== "") {
            validatedData[idx]["Negyedik_sor"] = ""; changed = true;
          }
          return; // ne írjuk vissza a beírt szöveget a Negyedik_sor mezőbe
        }
      }
