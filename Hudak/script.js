...
function getSelectedLogo() {
  ...
  return {
    src: selectedType === "A" ? "assets/hudak_logo.png" : "assets/hudak_logo2.png",
...
async function validateWithAgent(data, onComplete) {
  ...
  try {
    // Elküldjük a nyers adatokat → backend elvégzi a validálást
    // extract_kiszereles: ha a megnevezés végén van a kiszerelés, az agent kinyeri
    const response = await fetch(`${API_URL}/api/process-labels`, {
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ rows: data, subpage: "hudak" })
    });
...
function showValidationModal(validationResult, onComplete) {
  ...
  document.getElementById("applyFixesBtn").onclick = () => {
    ...
    logCorrections("hudak", overlay._loggedFixes, overlay._data, "applied");
    onComplete(overlay._data);
  };

  document.getElementById("skipValidationBtn").onclick = () => {
    overlay.classList.remove("active");
    logCorrections("hudak", overlay._loggedFixes, validationResult.processed_rows, "skipped");
...
function downloadTemplate() {
    ...
    link.href = "hudak_sablon.xlsx";
    link.download = "hudak_sablon.xlsx";
...
// =============================================================================
// CIMBI – természetes nyelvű címke-asszisztens (előnézet + visszavonás)
// =============================================================================
(function initCimbi() {
  const SUBPAGE = "hudak";
